...
#include "libcdatetime_narrow_string.h"
     ...
     libcerror_error_t **error )
{
	...
	static char *function  = "libcdatetime_narrow_string_split";
	...
	if( libcdatetime_narrow_split_string_initialize(
	     split_string,
	     string,
	     string_size,
	     number_of_segments,
	 ...
	 */
	if( libcdatetime_narrow_split_string_get_string(
	     *split_string,
	     &segment_start,
	     &string_size,
	     error ) != 1 )
	     ...
	     segment_index++ )
	{
		...
		if( segment_length >= 0 )
		{
			...
			if( libcdatetime_narrow_split_string_set_segment_by_index(
			     *split_string,
			     segment_index,
			     segment_start,
			     segment_length + 1,
		...
		if( segment_end == ( (libcdatetime_internal_narrow_split_string_t *) *split_string )->string )
		{
			segment_start++;
		}
		if( segment_end != ( (libcdatetime_internal_narrow_split_string_t *) *split_string )->string )
...
on_error:
	if( *split_string != NULL )
	{
		libcdatetime_narrow_split_string_free(
		 split_string,
		 NULL );
	}
	return( -1 );
