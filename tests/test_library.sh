...
LIBRARY_TESTS="elements error support timestamp";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./cdatetime_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libcdatetime" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
