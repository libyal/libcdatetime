# Script that builds libcdatetime
#
# Version: 20260608
...
Else
{
	...
	If (-Not (Test-Path "${VSSolutionPath}"))
	{
		...
		Invoke-Expression -Command "& '${Python}' ${MSVSCppConvert} --output-format ${VisualStudioVersion} ${VSToolsOptions} msvscpp\libcdatetime.sln 2>&1" | %{ "$_" }
	}
}
If (${VisualStudioVersion} -eq "2026")
{
	$VSSolutionFile = "${VSSolutionPath}\libcdatetime.slnx"
}
Else
{
	$VSSolutionFile = "${VSSolutionPath}\libcdatetime.sln"
}
